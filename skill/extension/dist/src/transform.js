...
function resolveNavigationMode(value, fallback = "0") {
    return value === "1" || value === "2" ? value : fallback;
}
function createDefaultSystemPrompt(enableExperimentalNativeActions = false) {
    const lines = [
...
function resolveToolCommand(toolName, options = {}) {
...
function decodeMarkerParams(rawValue) {
...
function extractLingzhuToolMarker(text) {
    ...
    if (!ALLOWED_MARKER_COMMANDS.has(command)) {
        return null;
    }
    return {
        command: command,
...
export function detectIntentFromText(text, options = {}) {
    ...
    if (markerMatch) {
        ...
        if (command === "take_navigation") {
            ...
            if (rawParams.destination)
                toolCall.poi_name = String(rawParams.destination);
            ...
            if (rawParams.title)
                toolCall.title = String(rawParams.title);
            if (rawParams.start_time)
                toolCall.start_time = String(rawParams.start_time);
            if (rawParams.end_time)
                toolCall.end_time = String(rawParams.end_time);
        }
        else if (command === "send_notification" || command === "send_toast" || command === "speak_tts") {
            if (rawParams.content)
                toolCall.content = String(rawParams.content);
            if (typeof rawParams.play_tts === "boolean")
                toolCall.play_tts = rawParams.play_tts;
            if (rawParams.icon_type)
                toolCall.icon_type = String(rawParams.icon_type);
        }
        else if (command === "start_video_record") {
            if (typeof rawParams.duration_sec === "number")
                toolCall.duration_sec = rawParams.duration_sec;
            if (typeof rawParams.width === "number")
                toolCall.width = rawParams.width;
            if (typeof rawParams.height === "number")
                toolCall.height = rawParams.height;
            if (typeof rawParams.quality === "number")
                toolCall.quality = rawParams.quality;
        }
        else if (command === "open_custom_view") {
            if (rawParams.view_name)
                toolCall.view_name = String(rawParams.view_name);
...
export function lingzhuToOpenAI(messages, context, options = {}) {
    const openaiMessages = [];
    const systemParts = [createDefaultSystemPrompt(options.enableExperimentalNativeActions === true)];
    ...
    if (context) {
        ...
        if (context.currentTime)
            parts.push(`当前时间: ${context.currentTime}`);
        if (context.location)
            parts.push(`位置: ${context.location}`);
        if (context.weather)
            parts.push(`天气: ${context.weather}`);
        if (context.battery)
            parts.push(`电量: ${context.battery}%`);
        if (context.latitude && context.longitude) {
            parts.push(`坐标: ${context.latitude}, ${context.longitude}`);
        }
        if (context.lang)
            parts.push(`语言: ${context.lang}`);
        if (context.runningApp)
            parts.push(`当前运行应用: ${context.runningApp}`);
        if (parts.length > 0) {
            openaiMessages.push({
                role: "system",
...
export function parseToolCallFromAccumulated(toolName, argsStr, options = {}) {
    const defaultNavigationMode = resolveNavigationMode(options.defaultNavigationMode);
    ...
    switch (command) {
        case "take_navigation":
            toolCall.action = args.action || "open";
        ...
        case "control_calendar":
            toolCall.action = args.action || "create";
            if (args.title)
                toolCall.title = String(args.title);
        ...
        case "speak_tts":
            if (args.content)
                toolCall.content = String(args.content);
            if (typeof args.play_tts === "boolean")
                toolCall.play_tts = args.play_tts;
            if (args.icon_type)
                toolCall.icon_type = String(args.icon_type);
            break;
        case "start_video_record":
            if (typeof args.duration_sec === "number")
                toolCall.duration_sec = args.duration_sec;
            if (typeof args.width === "number")
                toolCall.width = args.width;
            if (typeof args.height === "number")
                toolCall.height = args.height;
            if (typeof args.quality === "number")
                toolCall.quality = args.quality;
            break;
        case "open_custom_view":
            if (args.view_name)
                toolCall.view_name = String(args.view_name);
...
export function openaiChunkToLingzhu(chunk, messageId, agentId, options = {}) {
...
export function createFollowUpResponse(suggestions, messageId, agentId) {
    return {
        role: "agent",
        type: "follow_up",
        message_id: messageId,
...
export function extractFollowUpFromText(text, limit = 3) {
    const patterns = [
        /你还可以(?:问我|继续问|试试)[:：\s]*(.+)/,
        /(?:推荐|建议)(?:问题|提问)?[:：\s]*(.+)/,
        /(?:相关|更多)问题[:：\s]*(.+)/,
...
export function formatLingzhuSSE(event, data) {
